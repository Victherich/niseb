...
const UserLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${domain}/user_login.php`, { email, password });
      ...
      if (response.data.success) {
        ...
        // Dispatch login action with a single object containing both userInfo and userToken
  ...
  return (
    <Container>
      <Form onSubmit={handleLogin}>
        ...
        onClick={()=>navigate('/applicationform')}>Not a Member? Register here.</p>
        <p 
        style={{marginTop:"10px", cursor:"pointer"}}
        onClick={()=>navigate('/userforgotpassword')}>Forgot Password</p>
      </Form>
    </Container>
  );
};
