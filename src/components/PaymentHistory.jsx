...
//   const years = Array.from({ length: 1000 }, (_, i) => 2050 - i);
...
//                     <td>{t.email}</td>
//                     <td> {
//     membershipFees.find(item => item.id === Number(t.membershipCategory))?.name 
//     || "None"
//   }</td>
...
import styled from "styled-components";
import { Context } from "./Context";
import { FaCalendarAlt, FaMoneyBill, FaEnvelope, FaUser, FaArrowLeft, FaArrowRight, FaSearch } from "react-icons/fa";
...
const PaymentHistoryPage = () => {
  ...
  const fetchHistory = () => {
    setLoading(true);
    setError(null);
    fetch(`${domain}/get_payment_history.php`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setTransactions(data.transactions);
  ...
  return (
    <PageContainer>
      <ContentWrapper>
              ...
              <tbody>
                {filteredTransactions.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td>{t.email}</td>
                    <td>
                      {membershipFees.find(item => item.id === Number(t.membershipCategory))?.name || "None"}
                    </td>
        ...
        {!loading && !error && filteredTransactions.length === 0 && (
          <ErrorMsg>No transactions found.</ErrorMsg>
        )}
      </ContentWrapper>
    </PageContainer>
  );
